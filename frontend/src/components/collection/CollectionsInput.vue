...
import type {FileMetaId, FileMeta, Collection} from 'post-archiver'
...
const model = defineModel<Collection[]>({required: true})

const search = ref('')
const url = computed(() => `/api/collections` + (search.value ? `?search=${encodeURIComponent(search.value)}` : ''))

const {data} = useFetch(url, {
  refetch: true,
}).json<WithRelations<Totalled<Collection>>>()
