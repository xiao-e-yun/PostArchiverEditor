...
const url = computed(() => `/api/collections/${id.value}`);

const {data: raw} = useFetch(url, {refetch: true}).json<WithRelations<Collection>>();
...
const save = useCommonSaveAPI('collection', url, raw, changed, difference)
const remove = useRemoveAPI('collection', url)
</script>
...
<template>
  <div v-if="changed" class="flex gap-4 p-4 max-md:flex-col">
    ...
    <div class="flex-2 overflow-auto flex flex-col gap-4">
      <img v-if="changed.thumb" :src="parseFileMetaToUrl(fileMetas[changed.thumb]!)"
      alt="cover" class="aspect-video object-cover rounded-md">
      <PostList :collection="changed.id" />
    </div>
  </div>
</template>
